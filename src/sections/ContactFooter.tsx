...
const ContactFooter = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.name && formData.email && formData.message) {
      const contactEmail = config?.contactEmail || 'info@tagrugbyhk.org';
  ...
  const contactInfo = [
    { icon: Mail, label: 'Email', value: config?.contactEmail || 'info@tagrugbyhk.org' },
    { icon: Phone, label: 'Phone', value: config?.contactPhone || '+852 5795 1682' },
    { icon: Globe, label: 'Website', value: config?.websiteUrl || 'https://tagrugbyhk.org/', isLink: true },
  ];

  const socialLinks = [
    { icon: Instagram, url: config?.socialInstagram || 'https://instagram.com/hk.tagrugby', name: 'Instagram' },
    { icon: Youtube, url: config?.socialYouTube || 'https://youtube.com/@hktagrugby', name: 'YouTube' },
    { icon: Facebook, url: config?.socialFacebook || 'https://facebook.com/hktagrugby', name: 'Facebook' },
    { icon: Twitter, url: config?.socialTwitter || 'https://twitter.com/hk_tagrugby', name: 'Twitter' },
            ...
            <div className="flex flex-col lg:flex-row items-center justify-between gap-3 sm:gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-white/60 text-xs sm:text-sm">
                  {config?.tournamentName || 'Hong Kong Tag Rugby'}
                </span>
              </div>
