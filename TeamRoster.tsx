...
const TeamRoster = () => {
  ...
  useEffect(() => {
    ...
    const handleFilterTeams = (event: CustomEvent) => {
      const { region } = event.detail;
      if (region && region !== 'all') {
        // Map region to division prefix
        const regionToDivision: Record<string, string> = {
          'Hong Kong': 'HK',
          'Japan': 'JP',
          'AusAsia': 'AA'
        };
        const divisionPrefix = regionToDivision[region];
        if (divisionPrefix) {
          const filteredTeams = teams.filter(team => 
            team.division?.startsWith(divisionPrefix)
          );
    ...
    window.addEventListener('filterTeams', handleFilterTeams as EventListener);
    
    return () => {
      window.removeEventListener('filterTeams', handleFilterTeams as EventListener);
    };
  }, [teams]);
